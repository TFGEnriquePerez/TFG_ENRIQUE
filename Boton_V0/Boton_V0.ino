...
StaticJsonDocument<256> docCon,docEmer;

String jsonConS,docEmerS;
...
String ID = String(ESP.getChipId());
 ...
 void callback(char* topic, byte* payload, unsigned int length){
    ...
    if(strcmp(topic,"TFG/Boton/Emergencia")==0){ //Topic de configuracion que informa sobre el el periodo de envío de mensajes (en segundos), 
                                                   //el periodo de comprobación de actualizaciones en minutos y la velocidad de cambio de la salida PWM 
    // Deserialize the JSON document
      error = deserializeJson(docEmer,payload);
    // Compruebo si no hubo error
    if (error){ //Si ha habido error..
      Serial.print("Error deserializeJson() failed: ");
      Serial.println(error.c_str());
    }else if(docEmer.containsKey("Calibrar")){  // comprobar si existe el campo/clave que estamos buscando
        
        Emergencia = docEmer["Emergencia"];
        if(Emergencia==true){
          cuenta=3;
          }else{cuenta=0;}
           var1=false;
